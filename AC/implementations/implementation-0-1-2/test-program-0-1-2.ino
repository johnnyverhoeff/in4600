...
ISR(TIMER1_OVF_vect) {      // interrupt service routine 
  ...
  if (timer_enable == 1) {
    if (digitalRead(modulate_enable) == 0 && sample_idx < 16) {
  ...
  if (adc_buffer_full == 1) {
    ...
    /*for (int offset = 0; offset < (ADC_BUFFER_SIZE - L); offset++) {
      ...
      for (int i = 0; i < L; i++) {
        ...
        corr_sum += adc_buffer[i + offset] * r_chip;
      }

      float normalized_corr = (-2 * corr_sum - 1) / L / 850;

      Serial.println(normalized_corr);
